...
>
> 如果构建 `releases` 包，为了安全性签名开发者所持密钥不会被公开，在自行签名的情况下每次更新需要用同一组密钥重新构建安装包，无法直接更新开发者发布的 `.apk` 安装包
...
本应用与**小米公司（Xiaomi Corporation）及其关联公司（包括但不限于小米科技有限责任公司、北京小米移动软件有限公司等）**(下称小米公司)没有任何形式的隶属、授权、合作或关联关系**。**本应用并非小米公司的官方产品，也未获得小米公司的官方赞助、认可或背书**。

本应用中提及的“小米”、“Xiaomi”、“MIUI”、“澎湃OS (HyperOS)”、“米家”、“MiMo”等词汇及相关图形标识，**其商标权和知识产权均归小米公司所有**。本应用使用上述词汇仅出于描述产品兼容性、功能说明或指示用途的目的（即**“指示性合理使用”**），绝无任何侵犯小米公司知识产权的意图。

本应用为开源应用，提供的**功能和服务安全性需使用者自行审查**。因使用本应用而产生的任何数据丢失、设备异常、隐私泄露或其他直接或间接的损失，**均与开发者或小米公司无关**。**开发者及小米公司不对本应用的功能性、安全性及合法性承担任何形式的保证或赔偿责任**。**若在使用中遇到故障或缺陷，请在本项目的 [Issue](https://github.com/MRoldL001/MIMO-Chat/issues) 页进行反馈**，若有能力也可 fork 源码修改后发起 PR 来共同构建该应用。
